...
Support is monotonically decreasing with the length of the rule (Sec.~\ref{support}). Hence, whenever we apply an add-dangling-atom operator to a rule $R_p$ (the parent rule) to produce
...
This observation can be leveraged to speed up projection queries. The query for $supp(R_p)$ has one less join and thus executes faster.
...
The reason is that the calculation of both the standard confidence and the PCA confidence requires the calculation of the 
number of instantiations of the rule body. If the body contains atoms with many instantiations, the joins can be very expensive to compute.
...
This means that  a rule mining system might spend a significant amount of time to evaluate expensive confidence queries only to find out that the rule was of low confidence and of no practical use.
...
Our experiments (see Sec.~\ref{amiepm}) show that this technique prunes only 4\% of the rules erroneously. In return, it makes AMIE+ run in the range of minutes instead of days. It is thus one of the main techniques that allow AMIE+ to run on large-scale KBs.
...
Sec.~\ref{sec:appr} describes how the rule's body is approximated.
...
Finally, Sec.~\ref{sec:conf_upperBounds} derives upper bounds for the confidence of a particular class of rules.
...
higher runtimes and memory usage. An example is the rule that we saw above
\[
...
because such variables introduce many intermediate results. In our example, a director $x$ is related to many movies $z$ (the intermediate variable) that have different actors. 
...
In contrast, rules that do not contain existential variables (such as $livesIn(x,y)\wedge bornIn(x,y)\Rightarrow diedIn(x,y)$)  
...
are usually associated with more selective queries. In our examples, both $livesIn$ and $bornIn$ join on $x$ in the body and restrict the size of the result.

We will now see how to compute the approximation.
...
In some particular cases, we can derive lower bounds for the confidence denominator ($d_{pca}$, $d_{std}$) instead of using the approximation described in Sec.~\ref{sec:appr}.
...
d_{std} \ge \#x: \exists z:  r(x,z)  \label{eq:stdBoundDenom}
\end{equation}
This expression can be calculated in constant time with the indexes of our in-memory database (Sect.~\ref{subsec:implementation}).
