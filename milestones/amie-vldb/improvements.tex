...
% Given a rule and a maximum length, if there is no chance to close the rule with $n$ atoms, AMIE will stop refining the rule.
...
It follows that for a not-yet-closed rule of length $n-1$, AMIE will not apply 
...
rules with more than one non-closed variable are not refined with instantiated operators, 
because the application of this operator can close at most one variable with an atom.
...
\comment{Chris}{I rewrote the whole thing. I do not have any sql queries now. I also skip the notion or re-writing (perhaps we can remove it from the title). I only use re-writing in the
 example to explain the point. Check if what I day is actually correct and what AMIE is doing.}

Recall from Sec.~\ref{support} that support is defined also for intermediate rules that are not yet closed. 
...
For example, imagine the application of the operator $\mathcal{O}_D$ to the rule $R_p:livesIn(x,y)\Rightarrow citizenOf(x,y)$
produces a new rule $R_c: citizenOf(z,y)\wedge livesIn(x,y)\Rightarrow citizenOf(x,y)$. The addition 
of the dangling atom $citizenOf(z,y)$ cannot further restrict the support of $R_p$ because
the new atom is a less restrictive version of the atom $citizenOf(x,y)$. This means that $z$ will always 
bind to the same values as $x$. This implies that $R_c$ can be rewritten as 
\[
...
\noindent which is the same expression as $R_p$, the parent rule. Thus both $R_p$ and $R_c$ have the same support.
...
The fact that the child rule can be rewritten into to the parent rule can be exploited to speed up projection queries.
Recall that projection queries are used in AMIE to determine the new atoms that 
can be added to a rule in order to construct new rules without violating the support threshold.
...
\subsection{Exploit Confidence Thresholds}
...
\subsubsection{Approximating Confidence Denominator} \label{sec:appr}
...
respectively of the individual paths. However, we can use these upper bounds to approximate them.
