...
(i) contains a relation that already exists in another atom in the parent rule  and 
(ii) the same-relation-atoms have a variable in common.
...
Since $R_c$ has the same support with $R_p$, we can re-use from $R_p$ all the projection queries \comment{Chris}{@Luis: I say reuse the same queries
...
common variables of $R_c$ and $R_p$ ($x$ and $y$ in our example). Notice, though, that since $R_c$
contains one more variable than $R_p$ ($z$ in our example), AMIE cannot avoid creating the projection queries that involve binding new relations
with that variable.
...
The calculation of the  rules' confidence takes a significant part of the time of the overall rule-mining process (up to 29\% for YAGO2). 
This cost depends on properties of each rule. The confidence calculation (both standard and pca) involves the calculation of the 
...
between domains and ranges of various relations. 
These quantities
