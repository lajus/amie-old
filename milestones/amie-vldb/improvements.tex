Since the publication of the original AMIE framework~\cite{amie}, we have extended it with a series of improvements that allow the system to run over very large KBs.
...
Hence, in the following, we first discuss  improvements over AMIE that preserve the quality of the original results and then extensions based on approximations,
which can potentially lead to a lower recall.
% \comment{Katja}{Output is not exactly preserved as less rules are output. Text adapted accordingly.} \comment{Luis: }{I have made a couple of changes here. Have a look}
%In the first part of this section we discuss improvements over AMIE that preserve AMIE's original output. In the second part we discuss extensions based on approximations, which can potentially lead to a different output.
% In this section, we describe a series of improvements over the standard AMIE algorithm as it was initially presented in~\cite{amie}, which allows her to run over very large knowledge bases.
% In the following, we denote this new version of AMIE as AMIE+

%\subsection{Pruning by Bounds}
\subsection{Lossless Optimizations}
% \comment{Katja}{I changed the name of the subsection}
\label{prunebound}

% The techniques discussed in this subsection do not affect the quality of the result, i.e., runtime is improved, for instance, by applying pruning techniques
% that still produce the same results as AMIE and only low quality rules are pruned from the output that would not be useful to any application anyway.
...
% avoiding the run of unnecessary queries or taking advantage of c
% that cannot be output in any case and filter out rules that are expensive to calculate, but have a very low confidence value, by the use of a confidence upper bound which is cheaper to compute.
% \comment{Luis: }{I do not get it. The lossless optimizations are meant to prune
% intermediate results, not the output}
%\comment{Chris}{ok, I rewrote that have a look if you like it more} \comment{Luis}{Christina, let me know if you are ok with my rewrite}
...
% makes use of the maximum rule length to prune the search space even further.
...
\subsubsection{Perfect rules}
By definition, a rule cannot achieve a PCA confidence that is higher than 100\%. Thus, once a rule has achieved 100\% PCA confidence, we can stop adding new atoms. This is because the confidence cannot increase and the support can only decrease. Hence, any refinement is futile. We call rules with 100\% PCA confidence \emph{perfect rules}.
%A rule is perfect if it has 100\% PCA confidence. Since PCA confidence is not monotonic, the refinements of a perfect rule may not be perfect. This situation leads to a sub-optimal scenario where we obtain longer, more complex and less confident rules, having already a simpler and perfect hypothesis. For this reason, AMIE+ stops specializing perfect rules, i.e., once a rule is identified as perfect, it is not enqueued for further refinement.

\subsubsection{Query Rewriting and Caching}
In some cases, a query in the rule mining process contains redundant atoms. 
%\ignore{Fabian: rather than saying how bad AMIE is, let's say how cool AMIE+ is:
%AMIE relies on its own in-memory database implementation, which is optimized for the projection queries (Section~\ref{projectionQueries}).
%% required for the application of the different mining operators (line 9 in Algorithm~\ref{rm}).
%However, AMIE's database is not optimized for redundant atoms in queries (i.e., it does not implement any sort of query rewriting for performance purposes).
%Therefore, redundant atoms will still slow down the query response time even if the query accepts a much simpler formulation.
%In the following, we illustrate the problem and the solution of AMIE+ in terms of an example.
%}
...
%\comment{Luis: }{In the original AMIE paper we never mention thresholding on confidence, so I think we have to change the argument here.}
...
%Most applications of rule mining do not need low confidence rules. For instance, it does not make sense at all to make predictions based on a rule with 1\% confidence.
...
Hence, pruning based on confidence thresholds is applied only directly before the rules are output. Consequently, only the output is influenced, not the runtime.


\paragraph{Pruning} For AMIE+, we have developed a way to use minimum confidence thresholds to improve the performance of the system.  We use the confidence threshold to avoid evaluating expensive queries that would lead
to low confidence rules.
...
% \comment{Fabian}{Equation labeling does not work. In any case, equations and rules need some formatting}
% \comment{Katja}{Equation numbering seems to work now.}
...
%\comment{Katja}{Maybe continue the running example from above with some real numbers?} \comment{Luis}{The approximation is applied to a much broader set of rules, so I think it is fine to change the example.}
...
Our approximation formula is based on simple statistics, like the functionalities or the overlaps between domains and ranges of various relations. These quantities
...
Since support is always calculated for rules $R$ of the form $\vec{B}(x,y)\Rightarrow r_h(x,y)$, our aim is to derive a conservative approximation for $dnm_{pca}$ (or $dnm_{std}$) denoted by
...
\[
 dnm_{pca}(\vec{B} \Rightarrow r(x,y)) := {} \#(x,y): \exists z_1,...,z_m,y': \\ \quad \vec{B} \wedge r_h(x,y')
\]
...
I.e., the body produces $\#y_{per\; x} = 1/fun(\vec{B})$ facts per entity in its ``range'', on average. If we multiply, this number with the number of entities in $x$,
we get an estimate of body size of the rule. However, for the case PCA confidence, the denominator is restricted also by the entities in the domain of the head relation.
I.e., if we denote with $|dom(r)|$ the number of entities in the domain of relation $r$, our approximation will have the form:

\begin{equation} \label{eq:pcaApproxConf_general}
  \widehat{dnm}_{pca}(R):=|dom(\vec{B}) \wedge dom(r_h)|\cdot \#y_{per\; x}
\end{equation}

In the above formula, the critical quantities are the $dom(\vec{B})$ and the $\#y_{per\; x}$ and it is for these quantities that we would like to derive approximations.
However, using a confidence approximation to potentially prune rules is more effective for rules for which the exact confidence calculation is potentially expensive. 
For rules for which the exact confidence calculation is expected to be fast, the gain in runtime will be small and there is always some risk of false pruning.
Therefore we only use the confidence approximation for ``expensive'' rules. 
These rules typically have huge bodies ($(x,y)$ pairs) which flood the main memory. An example is the rule 
\[
 directed(x,z) \wedge hasActor(z,y) \Rightarrow married(x,y)
\]

This rule produces a huge body (it implies that a director is married to all actors that acted in his movies),
however its confidence is very low. Such huge bodies can only result through the introduction of the existential variables $z_1,...,z_m$ in  Eq.~\ref{eq:denomPCA} ($z$ in the example rule).
If a rule does not contain such existential variables, it can only further restrict the number of $(x,y)$ pairs produced by the body.
For example the rule $livesIn(x,y)\wedge bornIn(x,y)\Rightarrow diedIn(x,y)$ will have a smaller or equal body than the rule $bornIn(x,y)\Rightarrow diedIn(x,y)$.

In the following, we focus on rules for which the $x,y$ variables are connected through a chain of existential variables $z_1,..., z_{n-1}$. 
These rules can be written in the form:
Consider the following rule
$$
  r_1(x,z_1) \wedge r_2(z_1,z_2) \wedge ... \wedge r_n(z_{n-1},y) \Rightarrow r_h(x,y)
$$

Notice that in order to write a rule in this form we might need to replace some relations by their inverses
(e.g., substitute $r_2(z_2,z_1)$ with $r_2^{-1}(z_1,z_2)$)
or some atoms might need to be swapped.
A rule's body might contain many existential variables which might  define  multiple paths from $x$ to $y$. 
In the following, we will first explain how to compute the approximation for the case of a single path with one existential variable through a running example.
Then we generalize to rules with many existential variables, forming a single path and, in the end, we discuss the case of multiples paths.


\subsubsection{Approximate Confidence Denominator}

Let us consider again the  low confidence rule
\[
 directed(x,z) \wedge hasActor(z,y) \Rightarrow married(x,y)
\]
...
Therefore we take a walk through the path $x \rightarrow z \rightarrow y$, connecting a director to his potential actors. If $fun(r)$ and $ifun(r)$
...
\begin{enumerate} \itemsep +0.3ex
 ...
 \item Some or all of these movies will find join partners in the $z$ variable  of $hasActor$.
...
After step 2, we can approximate the number of distinct movies that bind to the variable $z$ for each director in the variable $x$ as:
$$
\#z_{per \; x} := \frac{|rng(directed) \wedge dom(hasActor)|}{|rng(directed)| \times fun(directed)}
$$
Here, $|rng(directed)|$ is the number of distinct movies in the range of $directed$ and $|rng(directed) \wedge dom(hasActor)|$ 
...
$$
The term $\frac{1}{fun(hasActor)}$ corresponds to step 3. At the end of step 3, we already have for a single director in $x$ a bag of actors in $y$ that correspond to it.
...
The term $\frac{1}{ifun(hasActor)}$ achieves this duplicate elimination: each actor contributes in the final count with a weight $ifun(hasActor)$, 
which expresses the extend to which this actor exclusively belongs to a movie.



In the way we use the duplicate elimination, a single actor $y$ belongs to $\frac{1}{ifun(hasActor)}$ different movies $z$, which are chosen from \emph{all} the movies in  $hasActor$ 
...
To summarize, we can estimate the number of actors $y$ that are supposed to be married with each director $x$ as:
$$
 \#y_{per\;x} :=  \#z_{per \; x} \times \#y_{ per \; z}
$$


\noindent To calculate $\widehat{dnm}_{pca}$ from Eq.~\ref{eq:pcaApproxConf_general}, we are now only missing the $dom(\vec{B})$. 
Here we make the simplifying assumption that $dom(\vec{B}) = dom(directed)$ and we calculate the number of distinct directors in $directed$ that join with the head relation $married$, 
i.e., $ov_{ss}(directed, married)$


To summarize, the $ \widehat{dnm}_{pca}(R)$ for a rule $r_1(x,z)\wedge r_2(z,y) \Rightarrow r_h(x,y)$ can be approximated by:

\[
  \widehat{dnm}_{pca}(R) := \frac{ |dom(r_1) \wedge dom(r_h)| \cdot  |rng(r_1) \wedge dom(r_2)|\cdot ifun(r_2)  }{|rng(r_1)| \cdot fun(r_2) \cdot  fun(r_1)}
\]
...
\begin{eqnarray*}
  \widehat{dnm}_{pca}(R) 	= \frac{|dom(r_1) \wedge dom(r_h)|}{fun(r_1)}  \times \\
  \prod_{i=2}^{n}\frac{|rng(r_{i-1})\wedge dom(r_i))|}{|rng(r_{i-1})|}\frac{ifun(r_i)}{fun(r_i)}
\end{eqnarray*}
...
the estimation of the expression $\#z_{per \; x}$ uses the term $\frac{|rng(r_1) \wedge dom(r_2)|}{|rng(r_1)|}$.
...
$$
  r_1(x_0,x_1) \wedge r_2(x_1,x_2) \wedge ... \wedge r_n(x_{n-1},x_n) \Rightarrow r_h(x_0,x_n)
$$
For this arrangement, some relations may have been replaced by their inverses, some atoms may have to be swapped, and some atoms may have to be omitted. There may be multiple paths from $x_0$ to $x_n$ for a given rule. However, since any path has at most as many atoms as the original rule, the support of the path rule will always be greater than the support of the original rule -- no matter which path we choose. This is an important property because we aim at overestimating confidence and strictly avoiding underestimations.
