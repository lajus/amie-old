...
\comment{Katja}{The following subsections should then logically be one level below the current one but this one already is a subsusection. So, maye rethinkt the structure of Secton 6.2}


\subsubsection{Expensive Rules}\label{sec:expensive_rules}

\comment{Katja}{This motivation and example is nice but I am not sure if it should be here. The previous section ends with ``now let's see how we can estimate\dots'', now this motivation is again on a more general level.}
