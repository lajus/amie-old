...
\comment{Chris}{In order to keep the search tractable and the experimental study manageable we ran AMIE with $maxLength=3$} \comment{Luis}{I crosschecked and there are 8 physical cores and 32 threads. }
...
To investigate the performance of AMIE/AMIE+ with longer rules, we ran both systems also with $maxLength=4$.  
...
The result is that mining on YAGO2 with $maxLength=4$ is 7.9x slower than on YAGO with $maxLength=3$ (see Table~\ref{amievsplus}).
