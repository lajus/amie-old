...
\hline
 ...
 KB			& AMIE				& Only 				& Only  		&Full   \\
  ...
  YAGO2 ($l=4$)  	& 24.99min  			&9.49min			& 26.48min 	  	& 8.35min 	 	    \\
  YAGO2s  		& $>$ 1 day  			&$>$ 1 day			&$>$ 1 day 	  	& 59.38min	  	    \\ 
  DBpedia 2.0  		& $>$ 1 day  			&$>$ 1 day			&$>$ 1 day 	  	& 46.88min	      	    \\ 
  DBpedia 3.8  		& $>$ 1 day  			&$>$ 1 day			&14h 10min*  	  	& 7h 6min 	     		\\
  Wikidata  		& $>$ 1 day  			&$>$ 1 day			&$>$ 1 day 	 	& 48.86min	  	    \\ \hline
\end{tabular}
\caption{Runtime  comparison of AMIE and AMIE+.}
\label{runtimeComparison}
\end{table*}
\end{savenotes}
\end{center}
...
Notice also that AMIE+(only Output) is only marginally faster than AMIE. Recall that the confidence approximation heuristic requires  computing the
...
\multirow{3}{*}{KB} 	& \multirow{3}{*}{AMIE}  	& \multicolumn{9}{c|}{AMIE+} 	\\ \cline{3-11}
  ...
  YAGO2 ($l=4$)  	& 24.99min  			&9.49min	& 1382		& 26.48min 			& 8.65min  			&15.69min 	&\textcolor{red}{26.39min}  	& 8.35min 	   & 645	& 100\%   	    \\ \hline
...
In this section we present an overview of the comparison between AMIE and two state-of-the-art rule mining systems 
...
\paragraph{Runtime}
For comparing WARMR with AMIE in terms of runtime, we tweaked AMIE to simulate WARMR's support and use the standard confidence
as quality metric for rules.  
WARMR was not able to terminate on YAGO2 in a time period of 1 day. Therefore, we created a sample of YAGO2 containing 47K triples
(see~\cite{amie} for details about the sampling method).
In that sample, AMIE mined her rules in 6.02 seconds on average accross 3 runs of the system.
WARMR, in contrast, took 18 hours. Both systems were run with an absolute support threshold of 5 entities.

We also ran both systems in a mode that allows them to mine rules with constants. For AMIE, it implies to enable
the instantiation operator $\mathcal{O}_I$ (see Section~\ref{subsubsec:refinement}).
AMIE completed the task in 1.43 minutes on average. WARMR for all relations did not terminate in 3 days.
...
We conclude that AMIE and AMIE+ are better suited for large KBs than WARMR.
...
AMIE+, in contrast, mined 75 closed rules, which included the ones mined by WARMR.
...
\paragraph{Runtime}
We ran AMIE and ALEPH on YAGO2. For ALEPH, we used the positive-only evaluation function with $Rsize=50$ 
...
AMIE terminated in 4.41 minutes and found rules for all relations. 
...
to remove this sentence as it may raise the reviewer's curiosity about the runtimes of ALEPH with constants. Do we have those times?}.
