...
\paragraph{Hardware}
All experiments are run on a server with \comment{Chris}{did this change?} 48GB of RAM and 8 CPUs (Intel Xeon at 2.4GHz). 
...
Except for YAGO (for which we explicitly use the type information), we removed the \emph{rdf:type} relationship because it inflates the size of the KBs. 
\comment{chris}{I do not like the above. It is like saying that types make it too large for us to process. How about the following?}
Types information was not used during mining in our runtime-performance experiments. 
...
This dataset contains additionally the \emph{rdf:type} statements of the entities in the YAGO core dataset. We restricted the types to those
in the domains and ranges of the relations. 
...
Moreover, AMIE uses as many threads as available cores in the system (8 in our hardware platform). \comment{chris}{if using a different blade, change also here}
...
introduced in Section~\ref{subsec:lossless}. 
In its default setting, AMIE+ has no confidence threshold and thus does not prune on confidence. 
...
These are lossless runtime enhacements that optimize the refinement phase
...
\multirow{3}{*}{KB} 	&  \multicolumn{3}{c|}{AMIE}  	& \multicolumn{5}{c|}{AMIE+} 	\\ \cline{5-9}
  ...
  YAGO2  		& 2.95min  	&	&	&29.37s		&			& 3.23min 	&    			& 28.19s	  	    \\ 
  YAGO2 (const)  	& 44.05min  	&	&	&11.72min	&			& 38.8min  	&   			& 9.93min	 	    \\
  YAGO2 ($l=4$)  	& 27.53min  	&	&	&9.49min	&			& 27.53min 	&   			& 8.35min 	 	    \\
...
\end{tabular}
\caption{Runtime  comparison of AMIE and AMIE+. Ref: average time for refinement phase among threads. Out: average time for output phase among threads.}
...
Table~\ref{runtimeComparison} shows the runtimes of AMIE and AMIE+. For AMIE+, we show results if only the improvements affecting the refinement process are active 
(maximum-rule-length, query-rewriting, perfect rules),
if only the improvements affecting the output process are active (confidence approximation, confidence upper bounds, both with confidence threshold 0.1) and if all improvements are active.
We also report the average time (among threads) needed for different phases of the mining (refinement and output).
First, notice that AMIE is not able to finish within a day for Yago2s, DBPedia 2.0 and 3.8 and Wikidata. 
For Yago2 (Yago2-const), we can see that AMIE+(full) is  6.3x (4.4x) faster than AMIE. This speedup is mainly due to the improvements targeting the refinement process:
...
for Yago2,  most of the time is spend in refinining rules and therefore by improving this phase we can see significant gain.
...
join cardinalities for every pair of relations in the KB. I.e., there is a tradeoff between an initial additional cost  for precomputing these values and the potential savings 
if the confidence approximation manages to prune some expensive rules. For the case of Yago2, the output phase takes only around 9\% of the overall mining time, i.e.,
...
\hline
			& AMIE			 	& \multicolumn{2}{c|}{AMIE+(full)} 	\\ \cline{2-4}
KB			& Rules 			& Rules		& Prun. prec. 	   \\ \hline
  YAGO2  		& 135				& 68 		& 100.00\%      	    \\ 
...
\end{tabular}
\caption{Output comparison of AMIE and AMIE+. *: output after processing for 1 day.}
...
Note that AMIE in our experiment was configured to run without any confidence threshold, where AMIE+ with confidence threshold 0.1.
I.e., normally we expect AMIE+ to output less rules than AMIE.
However, if a system ran for more than one day, we interrupted it
and considered the output until the point of interruption (noted with a ``*'' in Tab~\ref{outputComparison}).
This fact is particularly obvious
for DBpedia 2.0, where AMIE found only 26K rules, while AMIE+(full)  produces 112K rules.
...
As we can see, the speedup provided by AMIE+(full) does notil a se entarious decrease in the quality of the output: 
...
\multirow{3}{*}{KB} 	& \multirow{3}{*}{AMIE}  	& \multicolumn{9}{c|}{AMIE+} 	\\ \cline{3-11}
  ...
  YAGO2  		& 2.95min  			&29.37s		& 135		& 3.23min  & 29.03s   &         &  		& 28.19s	   & 135 	& 100\%      	    \\ \hline
  YAGO2 (const)  	& 44.05min  			&11.72min	& 19132		& 38.8min  & 8.90min  & 	&  		& 9.93min	   & 15634	& 100\% 	    \\ \hline  
  YAGO2 ($l=4$)  	& 27.53min  			&9.49min	& 1382		& 27.53min & 8.65min  & -       &  		& 8.35min 	   & 645	& 100\%   	    \\ \hline
  YAGO2s  		& $>$ 1 day  			&$>$ 1 day	& 247		&$>$ 1 day & 1.00h    & 	&  		& 59.38min	   & 94	 	& 100\%		    \\ \hline
  DBpedia 2.0  		& $>$ 1 day  			&$>$ 1 day	& 26812		&$>$ 1 day & - 	      & -       &  		& 46.88min	   & 112865	& 98.26\%      	    \\ \hline
  DBpedia 3.8  		& $>$ 1 day  			&$>$ 1 day	& 4931		& - 	   & - 	      & -       &  		& 7h 46min 	   & 5087       & 98.41\%	    \\ \hline
  Wikidata  		& $>$ 1 day  			&$>$ 1 day	& 1244		&$>$ 1 day & - 	      & -       &  		& 48.86min	   & 1650	& 96.93\%      	    \\ \hline
...
% Freebase 		& $>$ 1 day 			& $>$ 1 day & 182  & 131 		& $>$ 1 day & 5 			& 7.51min & 169 & 37 & 100\% \\ \hline
\end{tabular}
\caption{Runtime and output comparison between AMIE and AMIE+ on different KBs. Runtimes are averaged over 3 runs.}
\label{amievsplus}
\end{table*}
\end{savenotes}
\end{center}
...
In this section, we investigate the validity of the PCA as an assumption to generate counter-examples
for rule mining. Moreover, we revisit the comparison between the standard and the PCA confidence
as a ranking metric for rules and show how the type information can increase the predictive precision of rules.
...
The naive deduction approach presented in~\cite{amie} was designed as a proof of concept 
to show the suitability of the PCA confidence as precision metric for rules mined on a web-extracted KB. Yet, it
does not have a fully practical value, e.g., a precision of 35\% is certainly not high enough to make actual 
predictions about the death place, future spouse, or future residence of a person. We observe, however, that our 
inference approach disregards two important factors. First, in Section~\ref{PCA_assumption_entities} we discussed about granularity
...
the more rules make a prediction, the higher its confidence should be and we should therefore prioritize it for evaluation.
