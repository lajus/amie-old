...
% In general, we removed type information from the data sets. However for YAGO, we collected the types of the entities 
% according the YAGO3 ontology in a different dataset, since we used them for data prediction (Section~\ref{prediction}).
% We used the YAGO3 types because the type signatures in older versions of YAGO were too general for some relations.}
%the YAGO-type data set was obtained by adding the \emph{rdf:type} statements to the entities
...
% We restricted the \emph{rdf:type} statements to those types in the domains and ranges of the relations.
...
\comment{Chris}{In order to keep the search tractable and the experimental study manageable we ran AMIE with $maxLen=3$}
...
Table~\ref{timeProportion} shows the proportion of time spent by AMIE in each phase when running on YAGO2 
with and without constants (c).
...
\end{enumerate}
We also investigate the individual effects of the enhancements by enabling the output enhancements and one refinement enhancement at a time (columns +MRL, +QRW, +PR).
%We investigate the individual effects of the enhancements by enabling them one by one, i.e., the output enhancements and the refinement enhancements (+MRL, +QRW, +PR).
...
\paragraph{Longer rules}
To investigate the performance of AMIE/AMIE+ with longer rules, we ran both systems also with $maxLen=4$.  
First, we observe an increase of one order of magnitude in the number of rules output (645 vs. 68, see Table~\ref{outputComparison}). 
This provides a hint of how fast the search space increases for longer rules and how this affects the runtime, e.g., 
AMIE on YAGO2 with $maxLen=4$ is 9x slower than with $maxLen=3$ (see Table~\ref{amievsplus}).
% For AMIE+, we can observe a drop in the speedup offered over AMIE from 6.7x to 3x. 
In addition, we observe that the confidence approximation used in AMIE+ does not over-prune 
high-confidence rules (100\% pruning precision) as we move to longer rules.
...
As we can observe, AMIE with its default configuration is able to mine insightful rules on large KBs. We additionally 
show in Table~\ref{rules4atoms} some rules with 4 atoms mined on YAGO2.  
% the effect
% of increasing the maximum length of rules (parameter $maxLen$ in Algorithm~\ref{rm}) to 4 atoms. Table~\ref{rules4atoms} shows
% some examples.
...
\noindent where $D$ and $R$ correspond to the domain and range of the head relation $r$ in YAGO3.
We used the YAGO3 types because the type signatures in older versions of YAGO are too general 
and did not help us solve the granularity differences, e.g., the relation \emph{livesIn} is defined
from person to location on YAGO2s, whereas in YAGO3 it is defined from person to city. AMIE+ mined 62 typed rules
on a dataset consisting of YAGO2 and 2.29M \emph{rdf:type} statements about the subjects and objects of the triples
in YAGO2. 
...
Although this scoring-scheme is very simplistic (assumes independence of rules, confers a probabilistic interpretation to the confidence), it can still serve as  
a naive baseline for probabilistic inference methods. In practice, more involved methods~\cite{markovlogic,urdf} can be used with better results.
