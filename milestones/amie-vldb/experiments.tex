...
introduced in Section~\ref{subsec:lossless}. In its default setting, 
...
For AMIE, we show only the runtime. Since our goal is to observe the effect of each enhacement in the runtime of AMIE+, 
...
\multirow{3}{*}{KB} 	& \multirow{3}{*}{AMIE}  	& \multicolumn{8}{c|}{AMIE+} 	\\ \cline{3-10}
 			& 				& \multicolumn{2}{c|}{Default}	& \multicolumn{6}{c|}{AMIE + conf. approx}   \\ \cline{3-10}
			&  				&Runtime	& Rules 	& Runtime  & MRL      & MRL + QRW & Full 	   & Rules	& Prun. prec. 	   \\ \hline
  YAGO2  		& 2.95min  			&29.37s		& 135		& 	   &          &           & 28.19s	   & 135 	& 100\%      	    \\ \hline
  YAGO2 (const)  	& 	-  			&11.72min	& 19132		& 	   & 	      & 	  & 9.93min	   & 15634	& 100\% 	    \\ \hline  
  YAGO2 ($l=4$)  	& 27.53min  			&9.49min	& 1382		& - 	   & - 	      & -         & 8.35min 	   & 645	& 100\%   	    \\ \hline
  YAGO2s  		& $>$ 1 day  			&$>$ 1 day	&		&$>$ 1 day &  	      & 	  & 59.38min	   & 94	 	& 100\%		    \\ \hline
  DBpedia 2.0  		& $>$ 1 day  			&$>$ 1 day	&		& - 	   & - 	      & -         & 46.88min	   & 112865	& 98.26\%      	    \\ \hline
  DBpedia 3.8  		& $>$ 1 day  			&$>$ 1 day	&		& - 	   & - 	      & -         & 7h 46min 	   & 5087       & 98.41\%	    \\ \hline
  Wikidata  		& $>$ 1 day  			&$>$ 1 day	&		& - 	   & - 	      & -         & 48.86min	   & 1650	& 96.93\%      	    \\ \hline
...
In the following, we revisit from~\cite{amie} the comparison between the standard confidence measure 
and the PCA confidence measure as a ranking metric for rules.
...
In~\cite{amie}, we conducted a very naive inference approach to compare the standard and the PCA confidence 
...
\includegraphics[width=0.5\textwidth]{figures/std-vs-pca-conf-types}
\caption{Std. confidence vs. PCA confidence}
\label{finalshow}
\end{figure}

\paragraph{Using Type Information}
The experimental setup for data inference described above was designed as a proof of concept 
...
predictions about the death place, future spouse, or future residence of a person. We observe, however, that this naive
inference approach disregards two important factors. First, in Section~\cite{PCA_assumption_entities} we discussed about granularity
differences in relations. For instance, the relation $livesIn$ can link a person to her city or country of residence. 
This implies that for example, the rule 
$livesIn(x, y) \Rightarrow isCitizenOf(x, y)$ can conclude that some people are citizens of cities. Such spurious predictions decrease the precision
of the inference process.
Second, a prediction can be fired from multiple rules. This is the case for 9\% of the predictions drawn from the set of rules with more 
than 0.1 PCA confidence. The more rules make a prediction, the higher its confidence should be and we should prioritize it for evaluation.
...
\noindent where $D$ and $R$ correspond to the domain and range of the head relation $r$ according to the latest version of the
YAGO3\footnote{We used YAGO3 because the type signatures for older versions were too general for some relations.}. 
With a PCA confidence threshold of 0.1, AMIE found 57 typed rules on YAGO2.
This constraint suffices to solve the granularity differences discussed in Section~\cite{PCA_assumption_entities} as, for example, 
...
this is equivalent to rank predictions according to this score:
\[
...
\end{equation}
Equation~\ref{predictions-score} confers a probabilistic interpretation to the PCA confidence. The score of a prediction 
...
We note that Equation~\ref{predictions-score} assumes that rules are independent from each other. While this assumption may not be true sometimes, 
...
the rules were used to fire predictions and then the predictions were ranked by descending score and their cumulative precision
computed. Unlike the original experimental setup, the n-th point in the new curve corresponds to the cumulative precision
...
the original experiment. The fact that we can achieve 60\% confidence for 400K predictions, suggests that the PCA confidence in combination
with type checking constitute a promising tool for more sophisticated rule-based inference approaches.  
% While we cannot still add our predictions directly to a KB, such a simple approach guarantees than more than a half of the predictions
