We conduct three types of experiments: \comment{chris}{changed the order. Does it still make sense?}
In Section \ref{competitors}, we compare AMIE to competitor systems. 
We evaluate the number of rules and the prediction quality of the rules ion juxtaposition\comment{chris}{?}. 
In Section \ref{amiepm}, we compare AMIE with AMIE+. We show the performance gain for each of the new techniques from Section~\ref{sec:improvements}. 
We also conduct a proof of concept to show that the prediction quality can be enhanced by post-processing the results. 
Finally, in Section \ref{experimentspca}, we investigate the Partial Completeness Assumption (PCA). 
We evaluate how often the PCA actually holds in a real-world KB, and we also compare the PCA confidence with the standard confidence. 
...
\paragraph{Settings} Unless otherwise noted, we ran AMIE and AMIE+ in their default settings (Sec.~\ref{subsec:algorithm}), i.e., $minHC=0.01$, $maxLen=3$, and $minConf=XYZ$ for AMIE+. AMIE has no pruning on confidence, and thus $minConf=0$ for AMIE.
The systems use as many threads as available cores in the system (8 in our hardware platform). \comment{chris}{if using a different blade, change also here}
...
The instantiation operator $\mathcal{O}_I$ was disabled (``without constants'') unless mentioned otherwise.

\paragraph{Metrics}
We will compare AMIE and AMIE+ in terms of quality, ease of use and runtime to two popular state-of-the-art systems:
...
\subsection{Comparison of AMIE with state-of-the-art systems} \label{competitors}
...
\paragraph{Runtime}
For comparing WARMR with AMIE in terms of runtime, we tweeked AMIE to simulate WARMR's behavior in terms of the confidence measure used.  
...
Table \ref{warmrruntime} summarizes the runtime results for WARMR, AMIE, and AMIE+. 
...
\hline
 no 		& 18h 			& 7.91s 	& 5.24s \\
 yes 		& (48h) 	 	& 1.68min  	& 1.06min\\
\end{tabular}
\caption{Runtimes on YAGO2 sample\comment{Chris}{the numbers for AMIE+ are old and I do not know if we need them}}
\label{warmrruntime}
\end{table}
...
$Rsize$ is the total number of randoms, and $L$ is the number of atoms in the hypotheses. 
...
\paragraph{Runtime}
We ran AMIE and ALEPH on YAGO2 \cite{yago2}. For ALEPH, we used the positive-only evaluation function with $Rsize=50$ 
...
if only the improvements affecting the output process are active (confidence approximation, confidence upper bounds, both with confidence threshold 0.1) and if all improvements are active (Sec.~\ref{subsec:speedingConfidenceEvaluation}).
...
For Yago2 (Yago2-const), we can see that AMIE+(full) is  6.3x (4.4x) faster than AMIE. This speed-up is mainly due to the improvements that target the refinement process:
...
join cardinalities for every pair of relations in the KB. This means that there is a trade-off between an initial additional cost  for pre-computing these values and the potential savings. For the case of Yago2, the output phase takes only around 9\% of the overall mining time, i.e. the
...
This is particularly obvious for DBpedia 2.0, where AMIE found only 26K rules, while AMIE+(full)  produces 112K rules.
