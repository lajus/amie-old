...
% In this section we report about
% (i) usability and running time performance of AMIE in comparison to other state-of-the-art rule mining systems,
% (ii) the benefits in running time of AMIE from using the optimizations of Sec.~\ref{sec:improvements},
% (iii) the quality of rules produced when the PCA confidence is used as ranking metric in comparison to the standard confidence and the positives-only evaluation function (see Sec.~\ref{sec:pca}).
...
\paragraph{Settings}
Unless otherwise stated, AMIE uses a head coverage threshold of $\theta=1\%$ and
...
AMIE+ inherits this default behavior and extends it by
the lossless optimizations introduced in Section~\ref{prunebound}. In its parameter-less variant, which we will use in the following, AMIE+ has no confidence threshold and thus does not prune on confidence.
% Fabian: We use this only later. If an additional confidence threshold is given, AMIE+ can use it to prune the search space by bounds on the PCA confidence (see Section~\ref{upperBound}). We refer to this setting as \emph{AMIE+ with upper bounds}, and use a threshold of 0.1. In addition, AMIE+ can also be run with the lossy approximations enabled, a setting that we call \emph{AMIE+ with upper bounds and approximation}.
...
\comment{Chris}{I shortened this a lot. Please take a closer look. }
...
%, which enable us to run on very large datasets.
...
% \ignore{Fabian: This is obsolete, no? We already show that we can run on DBpedia.
% 
% \subsubsection{AMIE for Data Prediction}
% As a simple proof of concept for the value of rules for facts prediction in KBs, we ran AMIE
% on older versions of YAGO (YAGO2)\cite{yago2} and DBpedia (2.0)\cite{dbpedia}
...
% The results show that logical rules mined with the PCA confidence can be used to generate
...
%
% \paragraph{Lossless optimizations} They include maximum rule length, use of perfect rules for pruning, query rewriting and caching and pruning by confidence
% upper bounds. These improvements do not alter the completeness of AMIE, i.e., they do not prune the output, but still allow for faster mining.
% AMIE+ enables them by default. Note that the pruning by confidence upper bounds is applicable only when a minimum confidence threshold is provided. Since AMIE+ does not impose
% confidence thresholds by default, pruning by confidence upper bounds is disabled in the vanilla setting.
...
%
% \paragraph{Lossy optimizations} They include pruning by confidence approximations.
% These improvements might alter the expected output of AMIE since they prune rules that are expected to be low confident and very expensive to mine.
% They always require a minimum confidence threshold. For the type of rules discussed in Section~\ref{upperBound},
% the confidence upper bounds let us know precisely if the
% confidence of a rule will be below the confidence threshold, before calculating the actual confidence score. Our confidence
% approximations, on the other hand,
% provide a rough estimation of the confidence score which can be both an overestimation or underestimation of
% the actual confidence score. Underestimation affects the completeness of AMIE+,
% since some rules with high confidence score may be pruned. In any case, calculating the confidence upper bounds or estimating the confidence
% is much cheaper than computing the actual confidence. All the experiments involving confidence upper bounds or the lossy heuristics use a confidence
% threshold of 10\% PCA confidence.
%
%
% Table~\ref{amievsplus} shows the runtmimes of AMIE and AMIE+ on our test KBs. For the latter, we display
% the running time with different configurations: default AMIE+ (i.e., only lossless heuristics enabled, no confidence threshold), default AMIE+ with confidence
...
% pruned by the heuristics and the pruning precision. The pruning precision is the ratio of rules for which the confidence approximation
% introduced
% in Section~\ref{subsec:pruningbyapprox} would not err in pruning good rules. We
...
% and pruned nice rules.} \comment{Luis}{Isn't it the complement of the value we provide? If we overestimate 95\% of the times, then 5\% of the cases we are pruning a rule that we should not have pruned.}
% This means the value reported on Table~\ref{amievsplus}
% was calculated on a
% subset of all the rules and can be an approximation of the pruning precision. This fact is particularly obvious
% for DBpedia 2.0 where AMIE+ with default settings found
% only 12K rules over the 10\% threshold, whereas enabling all the heuristics produces 117K rules with PCA confidence greater or equals 10\%.
% \comment{Chris}{Are these rules in total or rules above 10\%? If it is the first let's compare it with the rules in total of AMIE+}. \comment{Luis}{The second case! I have updated the text accordingly.}
