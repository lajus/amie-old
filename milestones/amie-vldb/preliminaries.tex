...
% \comment{Chris}{at least for general-purpose web-extracted KBs, like YAGO, DBPEDIA and Freebase}.
...
% Fabian: Thanks! Looks good to me! I made some minor changes.
...
AMIE's language bias requires rules to be \emph{connected}. We say that two atoms in a rule are \emph{connected} if they share a variable or an entity.
...
\indented{
  \emph{R':} \emph{livesIn}$(x,y) \Rightarrow$ \emph{wasBornIn}$(y,z)$
} 
...
\ignore{ % Fabian: I tried to find another example without parallel universes
...
\www{
\paragraph{Head Coverage} \label{par:headCoverage}
