...
% \subsection{Existing Confidence Measures}
% \comment{Katja}{Maybe add a small intro here}
\subsection{Standard Confidence} \label{subsubsec:stdConf}
...
% Fabian: I rewrite to support, because we need this for the arguments in the improvement section
% \#(x,y): \exists z_1,...,z_m:
...
\subsection{The PCA Confidence}
In AMIE, we generate negative examples for a rule by the partial completeness assumption (PCA).
%We will now introduce this assumption, first introduce the assumption of partial completeness, before defining the measure, and evaluating the assumption.

\subsubsection{The Partial Completeness Assumption (PCA)}
...
This assumption is certainly true for functional relations $r$, such as \emph{birthdate} and \emph{capital}.
...
Even for other relations, the PCA is still reasonable for knowledge bases that have been extracted from a single source (such as DBpedia and YAGO), as we shall see in our qualitative analysis of the PCA in Section~\ref{pcaEvaluation}. The PCA has since been picked up by work on the Google Knowledge Vault under the name ``local completeness assumption'' \cite{knowledgevault}.

\subsubsection{The PCA Confidence}\label{subsubsec:pcaConf}
We propose to generate negative evidence by help of the PCA.
...
\end{small}
%
% Fabian: rewrite to "support" in order to be compliant with what we write in the improvement section
% \#(x,y): \exists z_1,...,z_m:
...
% \comment{Chris}{ @Fabian and Luis: Think about if you want to include the following one.}
...
% The PCA confidence can be seen as a process in which we first take a biased sample of the body (those facts containing entities for the $x$ variable that appear
% also in the head) and then calculate the standard confidence over this sample.
...
% However, our experiments show that overall PCA confidence identifies much more productive rules than the other measures
% and it also correlates with precision better than the standard confidence.
